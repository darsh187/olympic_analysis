...
def fetch__medal_tally(df,years,country):
...
def yearswise_medal_tally(df, country):
    temp_df = df.dropna(subset=['Medal'])
    temp_df.drop_duplicates(subset=['Team','NOC','Games','Year','City','Sport','Event','Medal'],inplace=True)
    newdf = temp_df[temp_df['region'] == country]
...
def country_event_heatmap(df, country):
    ...
    temp_df.drop_duplicates(subset=['Team','NOC','Games','Year','City','Sport','Event','Medal'],inplace=True)
    newdf = temp_df[temp_df['region'] == country]
    c_pt = newdf.pivot_table(index='Sport', columns='Year', values='Medal', aggfunc='count').fillna(0).astype('int')
    return c_pt

def most_succsessful_countrywise(df, country):
    temp_df = df.dropna(subset=['Medal'])
